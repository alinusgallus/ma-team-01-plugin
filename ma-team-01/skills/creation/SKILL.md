---
name: creation
description: Rédige les brouillons de posts d'une marque — soit un batch depuis le calendrier (« génère les posts de septembre pour Camille »), soit une idée à l'unité (« j'ai une idée de post pour … »). Écrit par analogie avec le corpus d'exemples de la marque et dépose les fichiers versionnés dans posts/. À utiliser pour toute demande de rédaction ou de réécriture de post, et pour retenir une correction durable de l'utilisatrice (« toujours plus court », « arrête les emojis », « retiens ça pour les prochains posts ») — invoquer même si la marque n'est pas nommée : le skill la demande. « Je veux prévoir / planifier un post pour plus tard » sans demande de rédaction immédiate = planning, pas creation. Ne publie ni n'envoie jamais rien : « publie », « valide le batch », « envoie » = gardefou.
---

# Creation — du sujet au brouillon, par analogie

Tu assembles chaque brouillon à partir de trois matières : un **sujet** (fourni par l'utilisatrice, jamais inventé), le **contexte** de la marque, et son **corpus**. Tu déposes tes fichiers dans `posts/`, et uniquement là — chaque écriture est une **création de fichier**, jamais une modification (conventions §1). Lis d'abord `${CLAUDE_PLUGIN_ROOT}/references/conventions.md` et `${CLAUDE_PLUGIN_ROOT}/references/vocabulaire.md`.
...
4. **Image nécessaire ?** (Instagram toujours ; LinkedIn si le sujet le réclame, si les Notes le précisent, ou si elle désigne un visuel — « avec la photo que je viens de mettre dans Drive », « avec mon design Canva … ») → appelle le skill **image** avec sa désignation : c'est lui qui rattache (jamais deviné — il nomme le fichier ou le design en retour pour qu'elle confirme), fait héberger et crée le constat dans `visuels/` (conventions §15). Puis renseigne dans l'en-tête ce qu'il te rend : `image` (le chemin du constat), `image_source`, `image_alt` — le texte alternatif est **à toi** en dernier ressort : une phrase qui décrit le visuel, dans la langue et le ton de la marque — et `image_url`. Une **photo jointe dans le chat** est un signal, pas une source : elle sert à identifier le fichier dans `images/` et à écrire l'alt text ; si elle n'est pas dans `images/`, message « photo dans le chat, pas dans Drive » d'`erreurs.md` et crée le post **sans visuel** en attendant. **À la première photo hébergée d'une marque** (aucun constat encore dans son `visuels/`), dis-lui, une fois : « Pour mettre une photo sur un post : partage-la depuis ta galerie vers le dossier `images` de la marque dans Drive, et dis-le-moi. Dès que je l'utilise, elle devient visible sur internet — même si le post ne part jamais. Et si tu retouches dans Drive une photo déjà utilisée, je te préviendrai qu'elle ne correspond plus au post. »
...
6. **Toute réécriture est une nouvelle version.** Pour une idée à l'unité (entrée B) comme après un retour du gardefou (« plus court », « moins de jargon ») : relis la version courante, réécris, et dépose `__v<N+1>` — le fichier précédent reste en place, la version la plus haute fait foi. Itère jusqu'à ce que l'utilisatrice soit satisfaite. **Un post qui reçoit un nouveau visuel est une nouvelle version, même si le texte ne change pas** : c'est elle qui porte le nouveau constat (conventions §15).

...
- **Post avec visuel parti sans `image_url`** (hébergement `aucun`) : une fois le fichier `planifie/` créé, guide l'utilisatrice — « Le texte est programmé dans Buffer ; le visuel est prêt dans Drive (<lien>). Ouvre le post dans Buffer et attache-le maintenant, pendant qu'on y est. » Ne stocke pas sa confirmation : le fichier `planifie/` ne se modifie jamais, et c'est **sync** qui vérifiera côté Buffer que le visuel est bien en place.
